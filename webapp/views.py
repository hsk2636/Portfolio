...
def delete_user_account(request):
    ...
    return redirect("profile_update")  # GET 요청은 프로필 업데이트 페이지로 리디렉션
...
def sign_in(request):
    if request.method == "POST":
        ...
        if user is not None:  # 인증 성공
            ...
            return redirect("main")  # 로그인 성공 후 리디렉션
        else:
            # 인증 실패 시 에러 메시지
...
def profile_update(request):
    if request.method == 'POST':
        user_userid = request.POST.get('user_userid')
        ...
        # 유효성 검사
        if not user_name or not user_address or not user_phoneNum or not confirm_password or not password:
        ...
        # 사용자 정보 업데이트
        user.user_userid = user_userid if user_userid else user.user_userid
        user.user_email = user_email if user_email else user.user_email
        ...
        try:
            user.save()
            # 비밀번호를 변경한 경우, 로그아웃 후 다시 로그인 요청
            if password:
                messages.success(request, "비밀번호가 변경되었습니다. 다시 로그인해주세요.")
                return redirect("sign_in")  # 로그인 페이지로 리다이렉트
            else:
                messages.success(request, "회원정보가 성공적으로 수정되었습니다.")
                return redirect("profile_update")  # 수정 페이지로 리다이렉트
        except Exception as e:
            messages.error(request, f"오류가 발생했습니다: {str(e)}")
            return redirect("profile_update")
...
def execute_geocode(request):
    if request.method == "POST":
        try:
            ...
            return JsonResponse({"message": "상품 최신화가 성공적으로 완료되었습니다."})
        except Exception as e:
