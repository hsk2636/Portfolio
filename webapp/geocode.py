...
# Django 설정을 로드하여 모델을 사용할 수 있게 함
application = get_wsgi_application()

# Django 모델 임포트
from dbapp.models import Users, Product

def save_user_locations_to_json():
    """ 사용자 주소와 제품 데이터를 포함하여 JSON 파일에 저장하는 함수 (무결성 검사 포함) """
    ...
    for user in Users.objects.all():
        ...
        for product in user_products:
            ...
            if (user.user_id, product.id) in existing_entries:
                print(f"Skipping: {user.user_address} for product {product.id} (already exists)")
                continue
            ...
            try:
                ...
                print(f"Request error for {user.user_address}: {e}")
            except Exception as e:
                print(f"Unexpected error for {user.user_address}: {e}")
